...
import 'schema/wholesale_record.dart';
import 'dart:async';
...
export 'schema/wholesale_record.dart';
...
Future<FFFirestorePage<SuppliersRecord>> querySuppliersRecordPage({
  ...
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, SuppliersRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      SuppliersRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<SuppliersRecord> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
...
/// Functions to query WholesaleRecords (as a Stream and as a Future).
Future<int> queryWholesaleRecordCount({
  Query Function(Query)? queryBuilder,
  int limit = -1,
}) =>
    queryCollectionCount(
      WholesaleRecord.collection,
      queryBuilder: queryBuilder,
      limit: limit,
    );

Stream<List<WholesaleRecord>> queryWholesaleRecord({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
    queryCollection(
      WholesaleRecord.collection,
      WholesaleRecord.fromSnapshot,
      queryBuilder: queryBuilder,
      limit: limit,
      singleRecord: singleRecord,
    );

Future<List<WholesaleRecord>> queryWholesaleRecordOnce({
  Query Function(Query)? queryBuilder,
  int limit = -1,
  bool singleRecord = false,
}) =>
...
Future<FFFirestorePage<WholesaleRecord>> queryWholesaleRecordPage({
  Query Function(Query)? queryBuilder,
  DocumentSnapshot? nextPageMarker,
  required int pageSize,
  required bool isStream,
  required PagingController<DocumentSnapshot?, WholesaleRecord> controller,
  List<StreamSubscription?>? streamSubscriptions,
}) =>
    queryCollectionPage(
      WholesaleRecord.collection,
      ...
      if (isStream) {
        final streamSubscription =
            (page.dataStream)?.listen((List<WholesaleRecord> data) {
          data.forEach((item) {
            final itemIndexes = controller.itemList!
                .asMap()
                .map((k, v) => MapEntry(v.reference.id, k));
